...
namespace JavaAsm.CustomAttributes {
    public class ConstantValueAttribute : CustomAttribute {
        ...
        internal override byte[] Save(ClassWriterState writerState, AttributeScope scope) {
            ...
            switch (this.Value) {
                case long longValue:
                    value = writerState.ConstantPool.Find(new LongEntry(longValue));
                    break;
                case float floatValue:
                    value = writerState.ConstantPool.Find(new FloatEntry(floatValue));
                    break;
                case double doubleValue:
                    value = writerState.ConstantPool.Find(new DoubleEntry(doubleValue));
                    break;
                case int integerValue:
                    value = writerState.ConstantPool.Find(new IntegerEntry(integerValue));
                    break;
                case string stringValue:
                    value = writerState.ConstantPool.Find(new StringEntry(new Utf8Entry(stringValue)));
                    break;
